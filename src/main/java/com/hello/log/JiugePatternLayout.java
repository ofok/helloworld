...
import com.hello.utils.JiugeUtils;
 ...
 * 脱敏插件
 */
@Plugin(name = "JiugePatternLayout", category = Node.CATEGORY, elementType = ELEMENT_TYPE, printObject = true)
@Slf4j
public class JiugePatternLayout extends AbstractStringLayout {


    private PatternLayout patternLayout;


    protected JiugePatternLayout(Charset charset, String pattern) {
        super(charset);
    ...
    private void initRule() {
        try {
            if (JiugeUtils.isEmpty(Log4j2Rule.regularMap)) {
                return;
            }
    ...
    public String hideMarkLog(String logStr) {
        try {
            //1.判断配置文件中是否已经配置需要脱敏字段
            if (StringUtils.isBlank(logStr) || JiugeUtils.isEmpty(KEY_REG_MAP) || JiugeUtils.isEmpty(REG_PATTERN_MAP)) {
                return logStr;
            }
            //2.判断内容是否有需要脱敏的敏感信息
    ...
    public static Layout createLayout(@PluginAttribute(value = "pattern") final String pattern,
        ...
        return new JiugePatternLayout(charset, pattern);
    }


    @Override
    public String toSerializable(LogEvent logEvent) {
        return hideMarkLog(patternLayout.toSerializable(logEvent));
    }
}
